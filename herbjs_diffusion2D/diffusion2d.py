...
def solve(dx = 0.1, dy = 0.1, D=4.):
    """Solve the diffusion problem in 2D

    Args:
        dx (float, optional): interval in x-direction, in mm. Defaults to 0.1.
        dy (float, optional): interval in y-direction, in mm. Defaults to 0.1.
        D (float, optional): Thermal diffusivity of steel, mm^2/s. Defaults to 4..
    """
    # plate size, mm
    w = h = 10.
    ...
    for n in range(nsteps):
        ...
        # Create figure
        if n in n_output:
            fig_counter, im = create_plot(fig, fig_counter, u, T_cold, T_hot, t=n*dt)
